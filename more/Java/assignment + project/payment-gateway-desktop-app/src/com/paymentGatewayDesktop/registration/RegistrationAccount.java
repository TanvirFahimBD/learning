...
public class RegistrationAccount extends JFrame {
	...
	private void initialize() {
		...
		this.setBounds(650, 200,450, 300);
	...
	private void doRegister() throws ClassNotFoundException {
		...
		try (Connection con = DriverManager.getConnection(
			...
			var passwordN = new String(passwordField.getPassword());
			var passwordR = new String(cPasswordField.getPassword());
			var queryEmail = "select email from public.reg_user where email =?";
			var pstmtE = con.prepareStatement(queryEmail);
			pstmtE.setString(1, emailTxtFld.getText());
			ResultSet resultSet = pstmtE.executeQuery();
			if (resultSet.next()){
